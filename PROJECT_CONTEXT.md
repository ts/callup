...
| 2026-08-19 | Consolidated repeated browser workflows | One JSON request boundary, action lifecycle, media-card builder, tracking mutation, episode release search, and candidate submission path now cover the repeated TV/movie UI behavior while keeping the frontend framework-free | Eighty tests; JavaScript syntax and diff checks; static Linux resource-bundle build and package verification |
